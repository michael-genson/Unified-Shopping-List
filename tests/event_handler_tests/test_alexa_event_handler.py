...
def test_alexa_event_handler_send_to_queue(api_client: TestClient, user_data: MockLinkedUserAndData):
    assert user_data.alexa_list.items

    list_event = build_alexa_list_event(
        Operation.create,
        ObjectType.list_item,
        list_id=user_data.alexa_list.list_id,
        list_item_ids=[item.id for item in user_data.alexa_list.items],
    )
    with mock.patch(fully_qualified_name(MockSQSFIFO.send_message)) as mocked_sync_handler:
        response = api_client.post(
            ...
            headers=get_auth_headers(user_data.user),
...
def test_alexa_event_handler_rate_limit(api_client: TestClient, user_data: MockLinkedUserAndData):
    assert user_data.alexa_list.items
